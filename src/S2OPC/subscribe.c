...
int main(int argc, char *const argv[])
{
    if (argc < 2)
    {
        printf("Usage: %s <nodeId> [<nodeId>] (e.g. %s \"i=2258\").\nThe '" DEFAULT_CONFIG_ID
...
#if 0
    extern SOPC_SecureConnection_Config *SOPC_ClientConfigHelper_CreateSecureConnection(const char *userDefinedId,
                                                                                        const char *endpointUrl,
                                                                                        OpcUa_MessageSecurityMode secuMode,
                                                                                        SOPC_SecurityPolicy_URI secuPolicy);

    const char *userDefineId = "userDefineId";
    const char *endpointUrl = "opc.tcp://127.0.0.1:4840";
    SOPC_SecureConnection_Config *readConnCfg = SOPC_ClientConfigHelper_CreateSecureConnection(userDefineId, endpointUrl,
    ...
    if (SOPC_STATUS_OK == status)
    {
        ...
        if (SOPC_STATUS_OK != status)
        {
            // when connecting open62541
            // 0x80860000 (Bad_SecureChannelClosed, The secure channel has been closed)
    ...
    /* Close the toolkit */
    fprintf(stderr, "<Example_wrapper_subscribe: SOPC_ClientConfigHelper_Clear\n");
    SOPC_ClientConfigHelper_Clear();

    fprintf(stderr, "<Example_wrapper_subscribe: SOPC_CommonHelper_Clear\n");
    SOPC_CommonHelper_Clear();

    fprintf(stderr, "<Example_wrapper_subscribe: end with status:%d\n", status);
